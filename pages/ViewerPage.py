...
class Viewer():
    ...
    def init_ladder(self):
        ...
        # Back Button
        Button(self.left_frame, text="<<< Back",
               command=self.goBack, bg='red').pack(side='top', fill='both')
    ...
    def init_page(self):
        ...
        # Body Frame for user options
        self.body_frame = LabelFrame(self.right_frame, text="Options")
        self.body_frame.pack(
            fill="both", expand="yes")

        # Most Active Player Button
        button_most_active_player = Button(self.body_frame, text="Most Active Player",
                                           command=self.getMostActivePlayer)
        button_most_active_player.grid(row=0, column=0, sticky="EW")

        # Least Active Player Button
        button_least_active_player = Button(self.body_frame, text="Least Active Player",
        ...
        button_historical_leaderboard = Button(self.body_frame, text="View Historical Leaderboard",
        ...
        self.date_entry = DateEntry(self.body_frame, width=12, background='darkblue',
        ...
        # footer_frame2.pack(fill='both')

        self.footer_frame = Frame(self.right_frame)
        self.footer_frame.pack(fill='both')

        button_all_matches = Button(self.footer_frame, text="All",
                                    command=self.getAllMatches)
        button_all_matches.grid(row=0, column=0, sticky="W")

        button_upcoming_matches = Button(self.footer_frame, text="Upcoming",
                                         command=self.getUpcomingMatches)
        button_upcoming_matches.grid(row=0, column=1, sticky="W")

        button_filter_by_name = Button(self.footer_frame, text="Filter by Name",
                                       command=self.getMatchesByName)
        ...
        button_filter_by_date = Button(self.footer_frame, text="Filter by Date",
                                       command=self.getMatchesByDate)
        button_filter_by_date.grid(row=0, column=4, sticky="W")

        # Historical Leaderboard Input
        self.date_entry_start = DateEntry(self.footer_frame, background='darkblue',
                                          foreground='white', borderwidth=2, date_pattern='dd/mm/y')
        self.date_entry_start.grid(row=0, column=5, sticky="W")

        # Historical Leaderboard Input
        self.date_entry_end = DateEntry(self.footer_frame, background='darkblue',
                                        foreground='white', borderwidth=2, date_pattern='dd/mm/y')
        ...
        self.upcoming_challenges.grid(
            column=0, row=1, columnspan=7)

        # Column Names
        self.upcoming_challenges["columns"] = (
            "Match Date", "Player 1", "Player 2", "Match 1", "Match 2", "Match 3", "Winner")
        ...
        for data in self.match_data:
            if data['action'] == 'result':
                ...
                self.upcoming_challenges.insert(
                    ...
                    'end',
                    text="",
                    values=(data['date'].strftime("%d-%m-%y"),
                            data['name_1'],
                            data['name_2'],
                            results[0],
                            results[1],
    ...
    def getMatchesByDate(self):
        ...
        self.resetTreeView(self.upcoming_challenges)
        for data in self.match_data:
            if data['date'].date() >= self.date_entry_start.get_date() and data['date'].date() <= self.date_entry_end.get_date():
                if data['action'] == 'result':
                    # Default Dictionary that returns '-' if key error
                    results = defaultdict(lambda: "-")
                    results[0] = '-'
    ...
    def getMatchesByName(self):
        ...
        self.resetTreeView(self.upcoming_challenges)
        for data in self.match_data:
            if data['name_1'] == self.name_filter.get() or data['name_2'] == self.name_filter.get():
                if data['action'] == 'result':
                    # Default Dictionary that returns '-' if key error
                    results = defaultdict(lambda: "-")
                    results[0] = '-'
    ...
    def getMostActivePlayer(self):
        ...
        most_active_player = [name for (name, total_matches) in self.matches_played.items(
        ) if total_matches == max_matches_played]
        self.results_1.set(f"Most Active Players: {str(most_active_player)}")

    def getLeastActivePlayer(self):
        min_matches_played = min(self.matches_played.values())
        least_active_player = [name for (name, total_matches) in self.matches_played.items(
        ) if total_matches == min_matches_played]
        self.results_1.set(f"Least Active Players: {str(least_active_player)}")
